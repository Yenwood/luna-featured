...
if (isset($_POST['add_rank'])) {
	...
	$db->query('INSERT INTO '.$db->prefix.'ranks (rank, min_comments) VALUES(\''.$db->escape($rank).'\', 0)') or error('Unable to add rank', __FILE__, __LINE__, $db->error());
...
elseif (isset($_POST['update'])) {
	...
	foreach ($rank as $item_id => $cur_rank) {
		...
		$db->query('UPDATE '.$db->prefix.'ranks SET rank=\''.$db->escape($cur_rank['rank']).'\', min_comments=\'0\' WHERE id='.intval($item_id)) or error('Unable to update ranks', __FILE__, __LINE__, $db->error());
	}
		...
		<div class="col-sm-4">
			<div class="panel panel-default">
                ...
                <div class="panel-body">
                    ...
                    <!-- <hr /> -->
                    <input type="hidden" class="form-control" name="new_min_comments" placeholder="<?php _e('Minimum comments', 'luna') ?>" maxlength="7" tabindex="2" value="0" hidden />
                </div>
			</div>
		</div>
	</form>
...
$result = $db->query('SELECT id, rank, min_comments FROM '.$db->prefix.'ranks ORDER BY id') or error('Unable to fetch rank list', __FILE__, __LINE__, $db->error());
