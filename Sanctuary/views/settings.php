...
<div class="main profile container">
	<div class="row">
		...
		?>
                                ...
                                <?php endif; if ($luna_user['is_admmod']):  // ADMIN ONLY STUFF LOL?>
                                    <div class="form-group">
							...
							<!-- yenwood stuff -->
									...
									<div class="form-group">
										<label class="col-sm-3 control-label"><?php _e('Choose user rank', 'luna') ?></label>
										<div class="col-sm-9">
											<div class="input-group">
												<select id="rank_id" class="form-control" name="rank_id">
													<?php
													$result = $db->query('SELECT * FROM '.$db->prefix.'ranks ORDER BY id DESC') or error('Unable to fetch user rank list', __FILE__, __LINE__, $db->error());

													while ($cur_rank = $db->fetch_assoc($result)) {
														if ($cur_rank['id'] == $user['rank'] || $user['rank'] == 0)
															echo "\t\t\t\t\t\t\t\t".'<option value="'.$cur_rank['id'].'" selected>'.luna_htmlspecialchars($cur_rank['rank']).'</option>'."\n";
														else
															echo "\t\t\t\t\t\t\t\t".'<option value="'.$cur_rank['id'].'">'.luna_htmlspecialchars($cur_rank['rank']).'</option>'."\n";
													}
													?>
												</select>
												<span class="input-group-btn">
													<input type="submit" class="btn btn-primary" name="update_rank" value="<?php _e('Save', 'luna') ?>" />
												</span>
											</div> <!-- .input-group -->
										</div> <!-- .col-sm-9 -->
									</div> <!-- .form-group -->
