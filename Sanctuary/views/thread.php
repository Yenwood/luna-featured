...
$this_thread = $db->query('SELECT * FROM '.$db->prefix.'threads WHERE id='.$cur_thread["first_comment_id"]);
while($r = $db->fetch_assoc($this_thread)){
    $cur_thread["featured"] = $r["featured"];
}
?>
		...
		<div class="col-sm-3">
			...
			<div class="list-group list-group-luna hidden-xs">
				...
				<?php if ($cur_thread['closed'] == '1') { ?>
					<a class="list-group-item list-group-item-danger" href="backstage/moderate.php?fid=<?php echo $cur_thread['forum_id'] ?>&open=<?php echo $id ?><?php echo $token_url ?>"><span class="fa fa-fw fa-lock"></span> <?php _e('Closed', 'luna') ?></a>
				<?php } else { ?>
					<a class="list-group-item list-group-item-success" href="backstage/moderate.php?fid=<?php echo $cur_thread['forum_id'] ?>&close=<?php echo $id ?><?php echo $token_url ?>"><span class="fa fa-fw fa-unlock"></span> <?php _e('Open', 'luna') ?></a>
				<?php } ?>
	
				<?php if ($cur_thread['pinned'] == '1') { ?>
					<a class="list-group-item list-group-item-success" href="backstage/moderate.php?fid=<?php echo $cur_thread['forum_id'] ?>&unpin=<?php echo $id ?><?php echo $token_url ?>"><span class="fa fa-fw fa-thumb-tack"></span> <?php _e('Pinned', 'luna') ?></a>
				<?php } else { ?>
					<a class="list-group-item" href="backstage/moderate.php?fid=<?php echo $cur_thread['forum_id'] ?>&pin=<?php echo $id ?><?php echo $token_url ?>"><span class="fa fa-fw fa-thumb-tack"></span> <?php _e('Unpinned', 'luna') ?></a>
				<?php } ?>
                ...
                <!-- Featured -->
				<?php if ($cur_thread['featured'] == '1') { ?>
					<a class="list-group-item list-group-item-success" href="backstage/moderate.php?fid=<?php echo $cur_thread['forum_id'] ?>&unfeature=<?php echo $id ?><?php echo $token_url ?>"><span class="fa fa-fw fa-exclamation"></span> <?php _e('Featured', 'luna') ?></a>
				<?php } else { ?>
					<a class="list-group-item" href="backstage/moderate.php?fid=<?php echo $cur_thread['forum_id'] ?>&feature=<?php echo $id ?><?php echo $token_url ?>"><span class="fa fa-fw fa-exclamation"></span> <?php _e('Unfeatured', 'luna') ?></a>
				<?php } ?>
			</div>
