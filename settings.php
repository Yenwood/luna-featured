...
if (isset($_POST['update_group_membership'])) {
	...
	$result = $db->query('SELECT group_id FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch ', __FILE__, __LINE__, $db->error());
...
if (isset($_POST['update_rank'])) {
	...
	$new_rank_id = intval($_POST['rank_id']);

	$result = $db->query('SELECT rank FROM '.$db->prefix.'users WHERE id='.$id) or error('Unable to fetch ', __FILE__, __LINE__, $db->error());
	$old_rank_id = $db->result($result);

	$db->query('UPDATE '.$db->prefix.'users SET rank='.$new_rank_id.' WHERE id='.$id) or error('Unable to change user rank', __FILE__, __LINE__, $db->error());
